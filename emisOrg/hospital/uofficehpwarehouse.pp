...
type
  ...
  { TfrmOfficeHPWarehouse }

  TfrmOfficeHPWarehouse = class(TbaseDbForm)
    actFind: TActionList;
    ...
    zqWarehouseOHPW_ADDRESS: TStringField;
    zqWarehouseOHPW_CODE: TStringField;
    zqWarehouseOHPW_DEPARTMENT: TLongintField;
    zqWarehouseOHPW_FAX: TStringField;
    zqWarehouseOHPW_ID: TLongintField;
    zqWarehouseOHPW_LOCATION: TLongintField;
    zqWarehouseOHPW_MAIL: TStringField;
    zqWarehouseOHPW_NAME: TStringField;
    zqWarehouseOHPW_PHONE: TStringField;
...
var
  frmOfficeHPWarehouse: TfrmOfficeHPWarehouse;
const
  EMPTY_SET : String = 'Prazan skup podataka.';
  {fields of tbl OfficeHPWarehouse(FW -> fields Warehouse) and view(office_hpwarehouse_v)}
  FW_ID : String = 'OHPW_ID';
  FW_CODE : String = 'OHPW_CODE';
  FW_NAME : String = 'OHPW_NAME';
  FW_LOCATION : String = 'OHPW_LOCATION';
  FW_ADDRESS : String = 'OHPW_ADDRESS';
  FW_PHONE : String = 'OHPW_PHONE';
  FW_FAX : String = 'OHPW_FAX';
  FW_MAIL : String = 'OHPW_MAIL';
  //VIEW  OFFICE_HPWAREHOUSE_V
...
procedure TfrmOfficeHPWarehouse.FormCreate(Sender: TObject);
begin
  {default arg - location(NAME)}
  locationArg:= FW_LOCATION_NAME;
end;

procedure TfrmOfficeHPWarehouse.dbgWarehouseMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgWarehouse.Cursor:= crHandPoint;
end;

procedure TfrmOfficeHPWarehouse.actFindLocationByCodeExecute(Sender: TObject);
begin
  locationArg:= FW_ZIP_CODE; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmOfficeHPWarehouse.actFindLocationByNameExecute(Sender: TObject);
begin
  locationArg:= FW_LOCATION_NAME; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmOfficeHPWarehouse.btnFindLocationClick(Sender: TObject);
begin
  {show popUpMnu}
  pupFindLocation.PopUp(Mouse.CursorPos.x, Mouse.CursorPos.y);
end;

procedure TfrmOfficeHPWarehouse.btnLocationCancelClick(Sender: TObject);
begin
...
procedure TfrmOfficeHPWarehouse.btnLocationOkClick(Sender: TObject);
begin
  {use current location}
  useThisLocation;
end;

procedure TfrmOfficeHPWarehouse.dbgLocationKeyDown(Sender: TObject;
...
procedure TfrmOfficeHPWarehouse.dbgLocationMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgLocation.Cursor:= crHandPoint;
end;

procedure TfrmOfficeHPWarehouse.dbgLocationTitleClick(Column: TColumn);
begin
  doSortDbGrid(TZAbstractDataset(TZAbstractRODataset(zroFindLocation)), Column);
end;

procedure TfrmOfficeHPWarehouse.dbgWarehouseTitleClick(Column: TColumn);
var
...
procedure TfrmOfficeHPWarehouse.dbLocationKeyPress(Sender: TObject;
  var Key: char);
begin
  {on pres Return}
  if(Key = #13) then
...
procedure TfrmOfficeHPWarehouse.edtGridSearchKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {Escape to exit}
...
procedure TfrmOfficeHPWarehouse.edtGridSearchKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmOfficeHPWarehouse.edtLocateEnter(Sender: TObject);
begin
...
procedure TfrmOfficeHPWarehouse.edtLocateExit(Sender: TObject);
begin
...
procedure TfrmOfficeHPWarehouse.edtLocateKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmOfficeHPWarehouse.FormClose(Sender: TObject;
...
procedure TfrmOfficeHPWarehouse.zqWarehouseAfterDelete(DataSet: TDataSet);
var
  recCount, recNo : String;
...
procedure TfrmOfficeHPWarehouse.zqWarehouseAfterOpen(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmOfficeHPWarehouse.zqWarehouseAfterPost(DataSet: TDataSet);
var
...
procedure TfrmOfficeHPWarehouse.zqWarehouseAfterScroll(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmOfficeHPWarehouse.saveBeforeClose;
var
  newDlg : TdlgConfirm;
...
procedure TfrmOfficeHPWarehouse.findLocation(textArg: String);
var
...
procedure TfrmOfficeHPWarehouse.useThisLocation;
begin
...
procedure TfrmOfficeHPWarehouse.onActFirst;
begin
  {jump to first rec}
  doFirstRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActPrior;
begin
  {jump to prior rec}
  doPriorRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActNext;
begin
  {jump to next rec}
  doNextRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActLast;
begin
  {jump to last rec}
  doLastRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActInsert;
begin
...
procedure TfrmOfficeHPWarehouse.onActDelete;
begin
  {delete rec}
  doDeleteRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActEdit;
begin
...
procedure TfrmOfficeHPWarehouse.onActSave;
begin
  {save rec}
  doSaveRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmOfficeHPWarehouse.onActCancel;
begin
  {hide panel}
  if(edtGridSearch.Visible) then
    edtGridSearch.Visible:= False;
...
procedure TfrmOfficeHPWarehouse.openDataSet;
begin
