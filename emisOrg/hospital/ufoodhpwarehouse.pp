...
type
  ...
  TfrmFoodHPWarehouse = class(TbaseDbForm)
    actFind: TActionList;
    ...
    zqWarehouseFHPW_ADDRESS: TStringField;
    zqWarehouseFHPW_CODE: TStringField;
    zqWarehouseFHPW_DEPARTMENT: TLongintField;
    zqWarehouseFHPW_FAX: TStringField;
    zqWarehouseFHPW_ID: TLongintField;
    zqWarehouseFHPW_LOCATION: TLongintField;
    zqWarehouseFHPW_MAIL: TStringField;
    zqWarehouseFHPW_NAME: TStringField;
    zqWarehouseFHPW_PHONE: TStringField;
...
var
  frmFoodHPWarehouse: TfrmFoodHPWarehouse;
const
  EMPTY_SET : String = 'Prazan skup podataka.';
  {fields of tbl FoodHPWarehouse(FW -> fields Warehouse) and view(food_hpwarehouse_v)}
  FW_ID : String = 'FHPW_ID';
  FW_CODE : String = 'FHPW_CODE';
  FW_NAME : String = 'FHPW_NAME';
  FW_LOCATION : String = 'FHPW_LOCATION';
  FW_ADDRESS : String = 'FHPW_ADDRESS';
  FW_PHONE : String = 'FHPW_PHONE';
  FW_FAX : String = 'FHPW_FAX';
  FW_MAIL : String = 'FHPW_MAIL';
  //VIEW  FOOD_HPWAREHOUSE_V
...
procedure TfrmFoodHPWarehouse.FormCreate(Sender: TObject);
begin
  {default arg - location(NAME)}
  locationArg:= FW_LOCATION_NAME;
end;

procedure TfrmFoodHPWarehouse.dbgWarehouseMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgWarehouse.Cursor:= crHandPoint;
end;

procedure TfrmFoodHPWarehouse.actFindLocationByCodeExecute(Sender: TObject);
begin
  locationArg:= FW_ZIP_CODE; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmFoodHPWarehouse.actFindLocationByNameExecute(Sender: TObject);
begin
  locationArg:= FW_LOCATION_NAME; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmFoodHPWarehouse.btnFindLocationClick(Sender: TObject);
begin
  {show popUpMnu}
  pupFindLocation.PopUp(Mouse.CursorPos.x, Mouse.CursorPos.y);
end;

procedure TfrmFoodHPWarehouse.btnLocationCancelClick(Sender: TObject);
begin
...
procedure TfrmFoodHPWarehouse.btnLocationOkClick(Sender: TObject);
begin
  {use current location}
  useThisLocation;
end;

procedure TfrmFoodHPWarehouse.dbgLocationKeyDown(Sender: TObject;
...
procedure TfrmFoodHPWarehouse.dbgLocationMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgLocation.Cursor:= crHandPoint;
end;

procedure TfrmFoodHPWarehouse.dbgLocationTitleClick(Column: TColumn);
begin
  doSortDbGrid(TZAbstractDataset(TZAbstractRODataset(zroFindLocation)), Column);
end;

procedure TfrmFoodHPWarehouse.dbgWarehouseTitleClick(Column: TColumn);
var
...
procedure TfrmFoodHPWarehouse.dbLocationKeyPress(Sender: TObject; var Key: char
  );
begin
  {on pres Return}
  if(Key = #13) then
...
procedure TfrmFoodHPWarehouse.edtGridSearchKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {Escape to exit}
...
procedure TfrmFoodHPWarehouse.edtGridSearchKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmFoodHPWarehouse.edtLocateEnter(Sender: TObject);
begin
...
procedure TfrmFoodHPWarehouse.edtLocateExit(Sender: TObject);
begin
...
procedure TfrmFoodHPWarehouse.edtLocateKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmFoodHPWarehouse.FormClose(Sender: TObject;
...
procedure TfrmFoodHPWarehouse.zqWarehouseAfterDelete(DataSet: TDataSet);
var
  recCount, recNo : String;
...
procedure TfrmFoodHPWarehouse.zqWarehouseAfterOpen(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmFoodHPWarehouse.zqWarehouseAfterPost(DataSet: TDataSet);
var
...
procedure TfrmFoodHPWarehouse.zqWarehouseAfterScroll(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmFoodHPWarehouse.saveBeforeClose;
var
  newDlg : TdlgConfirm;
...
procedure TfrmFoodHPWarehouse.findLocation(textArg: String);
var
...
procedure TfrmFoodHPWarehouse.useThisLocation;
begin
...
procedure TfrmFoodHPWarehouse.onActFirst;
begin
  {jump to first rec}
  doFirstRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActPrior;
begin
  {jump to prior rec}
  doPriorRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActNext;
begin
  {jump to next rec}
  doNextRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActLast;
begin
  {jump to last rec}
  doLastRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActInsert;
begin
...
procedure TfrmFoodHPWarehouse.onActDelete;
begin
  {delete rec}
  doDeleteRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActEdit;
begin
...
procedure TfrmFoodHPWarehouse.onActSave;
begin
  {save rec}
  doSaveRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmFoodHPWarehouse.onActCancel;
begin
  {hide panel}
  if(edtGridSearch.Visible) then
    edtGridSearch.Visible:= False;
...
procedure TfrmFoodHPWarehouse.openDataSet;
begin
