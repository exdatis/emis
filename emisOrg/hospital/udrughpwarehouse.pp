...
type
  ...
  TfrmDrugHPWarehouse = class(TbaseDbForm)
    actFind: TActionList;
    ...
    zqWarehouseDHPW_ADDRESS: TStringField;
    zqWarehouseDHPW_CODE: TStringField;
    zqWarehouseDHPW_DEPARTMENT: TLongintField;
    zqWarehouseDHPW_FAX: TStringField;
    zqWarehouseDHPW_ID: TLongintField;
    zqWarehouseDHPW_LOCATION: TLongintField;
    zqWarehouseDHPW_MAIL: TStringField;
    zqWarehouseDHPW_NAME: TStringField;
    zqWarehouseDHPW_PHONE: TStringField;
...
var
  frmDrugHPWarehouse: TfrmDrugHPWarehouse;
const
  EMPTY_SET : String = 'Prazan skup podataka.';
  {fields of tbl DrugHPWarehouse(FW -> fields Warehouse) and view(drug_hpwarehouse_v)}
  FW_ID : String = 'DHPW_ID';
  FW_CODE : String = 'DHPW_CODE';
  FW_NAME : String = 'DHPW_NAME';
  FW_LOCATION : String = 'DHPW_LOCATION';
  FW_ADDRESS : String = 'DHPW_ADDRESS';
  FW_PHONE : String = 'DHPW_PHONE';
  FW_FAX : String = 'DHPW_FAX';
  FW_MAIL : String = 'DHPW_MAIL';
  //VIEW  DRUG_HPWAREHOUSE_V
...
procedure TfrmDrugHPWarehouse.FormCreate(Sender: TObject);
begin
  {default arg - location(NAME)}
  locationArg:= FW_LOCATION_NAME;
end;

procedure TfrmDrugHPWarehouse.dbgWarehouseMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgWarehouse.Cursor:= crHandPoint;
end;

procedure TfrmDrugHPWarehouse.actFindLocationByCodeExecute(Sender: TObject);
begin
  locationArg:= FW_ZIP_CODE; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmDrugHPWarehouse.actFindLocationByNameExecute(Sender: TObject);
begin
  locationArg:= FW_LOCATION_NAME; //set search arg
  findLocation(dbLocation.Text); //set text arg
end;

procedure TfrmDrugHPWarehouse.btnFindLocationClick(Sender: TObject);
begin
  {show popUpMnu}
  pupFindLocation.PopUp(Mouse.CursorPos.x, Mouse.CursorPos.y);
end;

procedure TfrmDrugHPWarehouse.btnLocationCancelClick(Sender: TObject);
begin
...
procedure TfrmDrugHPWarehouse.btnLocationOkClick(Sender: TObject);
begin
  {use current location}
  useThisLocation;
end;

procedure TfrmDrugHPWarehouse.dbgLocationKeyDown(Sender: TObject;
...
procedure TfrmDrugHPWarehouse.dbgLocationMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  dbgLocation.Cursor:= crHandPoint;
end;

procedure TfrmDrugHPWarehouse.dbgLocationTitleClick(Column: TColumn);
begin
  doSortDbGrid(TZAbstractDataset(TZAbstractRODataset(zroFindLocation)), Column);
end;

procedure TfrmDrugHPWarehouse.dbgWarehouseTitleClick(Column: TColumn);
var
...
procedure TfrmDrugHPWarehouse.dbLocationKeyPress(Sender: TObject; var Key: char
  );
begin
  {on pres Return}
  if(Key = #13) then
...
procedure TfrmDrugHPWarehouse.edtGridSearchKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {Escape to exit}
...
procedure TfrmDrugHPWarehouse.edtGridSearchKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmDrugHPWarehouse.edtLocateEnter(Sender: TObject);
begin
...
procedure TfrmDrugHPWarehouse.edtLocateExit(Sender: TObject);
begin
...
procedure TfrmDrugHPWarehouse.edtLocateKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmDrugHPWarehouse.FormClose(Sender: TObject;
...
procedure TfrmDrugHPWarehouse.zqWarehouseAfterDelete(DataSet: TDataSet);
var
  recCount, recNo : String;
...
procedure TfrmDrugHPWarehouse.zqWarehouseAfterOpen(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmDrugHPWarehouse.zqWarehouseAfterPost(DataSet: TDataSet);
var
...
procedure TfrmDrugHPWarehouse.zqWarehouseAfterScroll(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmDrugHPWarehouse.saveBeforeClose;
var
  newDlg : TdlgConfirm;
...
procedure TfrmDrugHPWarehouse.findLocation(textArg: String);
var
...
procedure TfrmDrugHPWarehouse.useThisLocation;
begin
...
procedure TfrmDrugHPWarehouse.onActFirst;
begin
  {jump to first rec}
  doFirstRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActPrior;
begin
  {jump to prior rec}
  doPriorRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActNext;
begin
  {jump to next rec}
  doNextRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActLast;
begin
  {jump to last rec}
  doLastRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActInsert;
begin
...
procedure TfrmDrugHPWarehouse.onActDelete;
begin
  {delete rec}
  doDeleteRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActEdit;
begin
...
procedure TfrmDrugHPWarehouse.onActSave;
begin
  {save rec}
  doSaveRec(TZAbstractDataset(zqWarehouse));
end;

procedure TfrmDrugHPWarehouse.onActCancel;
begin
  {hide panel}
  if(edtGridSearch.Visible) then
    edtGridSearch.Visible:= False;
...
procedure TfrmDrugHPWarehouse.openDataSet;
begin
