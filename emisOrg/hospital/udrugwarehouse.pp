...
procedure TfrmDrugWarehouse.edtGridSearchKeyDown(Sender: TObject;
...
procedure TfrmDrugWarehouse.edtGridSearchKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmDrugWarehouse.dbgLocationKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {Ctrl+space to use this result}
  if (ssCtrl in Shift)and (Key = VK_SPACE) then
