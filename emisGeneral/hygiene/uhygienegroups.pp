...
type
  ...
  TfrmHygieneGroups = class(TbaseDbForm)
    dbCode: TDBEdit;
    dbgGroups: TDBGrid;
    dbName: TDBEdit;
...
var
  frmHygieneGroups: TfrmHygieneGroups;
const
  {fields of tbl hygiene_group}
  FG_ID : String = 'HG_ID';
...
procedure TfrmHygieneGroups.dbgGroupsMouseMove(Sender: TObject;
...
procedure TfrmHygieneGroups.dbgGroupsTitleClick(Column: TColumn);
var
...
procedure TfrmHygieneGroups.edtLocateEnter(Sender: TObject);
begin
...
procedure TfrmHygieneGroups.edtLocateExit(Sender: TObject);
begin
...
procedure TfrmHygieneGroups.edtLocateKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmHygieneGroups.FormClose(Sender: TObject;
...
procedure TfrmHygieneGroups.ztGroupsAfterDelete(DataSet: TDataSet);
var
  recCount, recNo : String;
...
procedure TfrmHygieneGroups.ztGroupsAfterOpen(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmHygieneGroups.ztGroupsAfterPost(DataSet: TDataSet);
var
...
procedure TfrmHygieneGroups.ztGroupsAfterScroll(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmHygieneGroups.saveBeforeClose;
var
  newDlg : TdlgConfirm;
...
procedure TfrmHygieneGroups.onActFirst;
begin
  {jump to first rec}
  doFirstRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActPrior;
begin
  {jump to prior rec}
  doPriorRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActNext;
begin
  {jump to next rec}
  doNextRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActLast;
begin
  {jump to last rec}
  doLastRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActInsert;
begin
...
procedure TfrmHygieneGroups.onActDelete;
begin
  {delete rec}
  doDeleteRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActEdit;
begin
...
procedure TfrmHygieneGroups.onActSave;
begin
  {save rec}
  doSaveRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.onActCancel;
begin
  {cancel rec}
  doCancelRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmHygieneGroups.openDataSet;
begin
