...
type
  ...
  TfrmAppliancesGroup = class(TbaseDbForm)
    dbCode: TDBEdit;
    dbgGroups: TDBGrid;
    dbName: TDBEdit;
    ...
    ztGroups: TZTable;
    ztGroupsAG_CODE: TStringField;
    ztGroupsAG_ID: TLongintField;
    ztGroupsAG_NAME: TStringField;
...
var
  frmAppliancesGroup: TfrmAppliancesGroup;
const
  {fields of tbl appliances_group}
  FG_ID : String = 'AG_ID';
  FG_CODE : String = 'AG_CODE';
  FG_NAME : String = 'AG_NAME';
implementation
uses
  uDModule, uConfirm;
{$R *.lfm}
...
procedure TfrmAppliancesGroup.dbgGroupsMouseMove(Sender: TObject;
...
procedure TfrmAppliancesGroup.dbgGroupsTitleClick(Column: TColumn);
var
...
procedure TfrmAppliancesGroup.edtLocateEnter(Sender: TObject);
begin
...
procedure TfrmAppliancesGroup.edtLocateExit(Sender: TObject);
begin
...
procedure TfrmAppliancesGroup.edtLocateKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {try to locate}
...
procedure TfrmAppliancesGroup.FormClose(Sender: TObject;
...
procedure TfrmAppliancesGroup.ztGroupsAfterDelete(DataSet: TDataSet);
var
  recCount, recNo : String;
...
procedure TfrmAppliancesGroup.ztGroupsAfterOpen(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmAppliancesGroup.ztGroupsAfterPost(DataSet: TDataSet);
var
...
procedure TfrmAppliancesGroup.ztGroupsAfterScroll(DataSet: TDataSet);
var
  recCount, recNo : String;
  recMsg : String = '0 od 0';
begin
...
procedure TfrmAppliancesGroup.saveBeforeClose;
var
  newDlg : TdlgConfirm;
...
procedure TfrmAppliancesGroup.onActFirst;
begin
  {jump to first rec}
  doFirstRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActPrior;
begin
  {jump to prior rec}
  doPriorRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActNext;
begin
  {jump to next rec}
  doNextRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActLast;
begin
  {jump to last rec}
  doLastRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActInsert;
begin
...
procedure TfrmAppliancesGroup.onActDelete;
begin
  {delete rec}
  doDeleteRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActEdit;
begin
...
procedure TfrmAppliancesGroup.onActSave;
begin
  {save rec}
  doSaveRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.onActCancel;
begin
  {cancel rec}
  doCancelRec(TZAbstractDataset(ztGroups));
end;

procedure TfrmAppliancesGroup.openDataSet;
begin
