...
begin
  TZAbstractDataset(ztDocSOInput).DisableControls;
  TZAbstractDataset(ztDocSOInput).Close;
  try
    TZAbstractDataset(ztDocSOInput).Open;
    TZAbstractDataset(ztDocSOInput).EnableControls;
  except
    on e : Exception do
    begin
